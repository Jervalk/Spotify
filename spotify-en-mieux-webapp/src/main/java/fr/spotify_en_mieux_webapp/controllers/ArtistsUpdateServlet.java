...
@MultipartConfig
public class ArtistsUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setAttribute("artist", as.findById(Integer.parseInt(request.getParameter("id"))).get());
		request.setAttribute("artists", as.findAll());
		request.setAttribute("groups", as.findAllGroups());
		this.getServletContext().getRequestDispatcher("/WEB-INF/jsp/pages/artistsUpdate.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "PERSON":
			...
			if (groupsIds != null)
				a.setGroups(Arrays.stream(groupsIds)
					.map(i -> ((Group) as.findById(Integer.parseInt(i)).get()))
		...
		case "GROUP":
			...
			if (membersIds != null)
				((Group)a).setMembers(Arrays.stream(membersIds)
					.map(i -> as.findById(Integer.parseInt(i)).get())
					.collect(Collectors.toSet()));
			break;
		default:
		...
		if (picture != null && picture.getSize() > 0) {
			...
			a.setPicture(as.findById(id).get().getPicture());
		}
		// validating artist
