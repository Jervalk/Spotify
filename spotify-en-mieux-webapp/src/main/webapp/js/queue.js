...
class Queue {
    ...
    constructor(songs) {
		...
		// loading queue data from local storage
		let storage = window.localStorage.getItem("queue");
		let data = JSON.parse(storage);
		this.currentSong = data.currentSong;
		data.songs.forEach(s => this.add(s));
    ...
    add(song) {
        ...
        // create and fill HTML element
        let element = this.createQueueItem(song, this.songs.length);
    ...
    bind(element) {
        ...
        element.querySelectorAll(".queue-add").forEach(b => {
            b.addEventListener("click", e => {
            	let s = b;
            	while (!((s = s.parentNode).classList.contains("song")));
                this.add(songFromHtml(s));
            });
        });
        ...
        element.querySelectorAll(".queue-insert-play").forEach(b => {
            b.addEventListener("click", e => {
            	let s = b;
            	while (!((s = s.parentNode).classList.contains("song")));
                this.insertAndPlay(songFromHtml(s));
            });
        });
